	...
	<div id="main">
		<div class="header">
			...
			<div class="header-center">
				<a href="${pageContext.request.contextPath}/CourseController?action=selectCourseView&registNumber=<%=session.getAttribute("username") %>">Grades List Per Course</a>
				<a class="active">Assign Grades Per Course</a>
			</div>
		        ...
		        <tbody>
		           	<c:forEach items="${requestScope.GradesCourse}" var="s">
		                <tr>
		                    ...
		                    <td><a href="${pageContext.request.contextPath}/ProfessorController?action=addGrades&IdCourse=<c:out value="${s.course.idcourse}"/>&RegistNumber=<c:out value="${s.student.registrationNumber}"/>">Add Grades</a></td>
		             	</tr>
		            </c:forEach>
	        	</tbody>
			</table>
